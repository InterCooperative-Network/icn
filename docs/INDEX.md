...
- [inbound-handshake-cancellation-2521.md](architecture/inbound-handshake-cancellation-2521.md) - Causal record for #2521: why a shutdown-polling deadline must never bound a QUIC/TLS handshake that has already arrived, the deterministic one-poll reproducer, and the disproved hypotheses
...
- [network-session-identity-binding.md](architecture/network-session-identity-binding.md) - A connection may be attributed to a remote DID only if that DID authenticated the certificate presented by that exact connection: the three jointly-required facts, why the absent-certificate path fails closed, and why a failed claim must never penalise the claimed DID
- [protocol-state-migration-invariants.md](architecture/protocol-state-migration-invariants.md) - A replay high-water is meaningful only inside the receiver replay semantics AND the authenticated sender sequence semantics that produced it (#2517): why no local history does not prove no legacy namespace, why a current capability cannot classify a historical envelope, why first establishment costs one retirement horizon, and why downgrade and unknown-future both fail closed
- [protocol-state-migration-mutation-controls.md](architecture/protocol-state-migration-mutation-controls.md) - Which deliberate defect each #2517 test actually catches, recorded so a change that makes one vacuous shows up as a gap rather than a still-green suite; includes two harness self-corrections
