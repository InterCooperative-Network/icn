...
| `ProcessGateResultReceipt` | fields `session_id, domain_id, gate_kind: ProcessGateKind, result: ProcessGateResult, recorded_by, recorded_at, record_hash`; tag `icn:gov:process_gate_result:v1`; `result` is a two-variant `Pass` / `Fail` enum; anchored on `(session_id, domain_id, gate_kind, result, recorded_by)`; `record_hash` is the sole equality anchor | `proof.rs` (`ProcessGateResultReceipt`) |
