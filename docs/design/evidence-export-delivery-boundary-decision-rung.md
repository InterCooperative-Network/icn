...
| the eight landed classes | `ProcessSessionOpened` / `DeliberationEntryRecorded` / `DecisionRecorded` / `ProcessGateResult` / `ActivationCrossed` / `MutationPlanRecorded` / `MutationApplied` / `EvidencePacketProduced` are the only runtime `ProcessTransitionReceipt`s | `proof.rs` |
