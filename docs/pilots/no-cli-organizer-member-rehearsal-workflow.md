...
| Completing an action item | **Action item** status change + **ActionItemCompletionReceipt** | Proof loop; read-side retrieval where documented (see [runtime-surface-map.md](../reference/project-index/runtime-surface-map.md)) |
...
- **UI prototype** — Single guided shell (organizer mode) with the §3 screen list.
  - **Read-only preview slice (landed):** [#2237](https://github.com/InterCooperative-Network/icn/pull/2237) adds a fixture-only **Review Preview** tab to `web/pilot-ui/?mode=demo`. It renders the `preview-review` wrapper and canonical-matched pending-publish rows in plain language, including organizer-readable assignee labels, conditional review affordances, mutation consequences, and expected receipt categories. Every review choice is disabled; the surface records no decision, performs no assignment or mutation, creates no ActionCard, and issues no receipt/evidence. It demonstrates step 3 and read-only information relevant to steps 5–6; it does not implement step 4 or complete the nine-step shell or the accessibility/organizer-rehearsal gates.
- **Fixture-backed demo** — Deterministic fixture load with no network by default.
  - **Bridge slice (landed):** a repo-safe read-model bundle the shell loads in demo mode lives at [`rehearsal-shell.manifest.json`](../../web/pilot-ui/fixtures/icn-organizer-demo/rehearsal-shell.manifest.json) (extending the `?mode=demo` fixture pack with the `preview-review` `preview_kind: pending_publish_summary` wrapper, the `pending-publish-summary` rows, and the `rehearsal-evidence-export` packet, alongside the existing standing / Action Card fixtures). It is validated **offline** by [`validate-rehearsal-shell-fixtures.py`](../scripts/validate-rehearsal-shell-fixtures.py), which asserts demo-mode / network-disabled, runs each packet through the existing contract validators, checks the browser-served row copy against the canonical example, and runs a deterministic forbidden live/private-reference guard. The wrapper and rows are composed **by URN, not nested** (per [`pending-publish-summary.md`](../contracts/pending-publish-summary.md)).
  - **Still open:** the full backend `--demo-mode` flag on icnd that [ICN#1727](https://github.com/InterCooperative-Network/icn/issues/1727) tracks remains **OPEN**; [ICN#1726](https://github.com/InterCooperative-Network/icn/issues/1726) remains open for the complete organizer shell and walkthrough. The generic private-overlay / DID activation **spec** landed under [ICN#1730](https://github.com/InterCooperative-Network/icn/issues/1730), but Review Preview deliberately performs no label-to-DID binding and claims no activation implementation.
