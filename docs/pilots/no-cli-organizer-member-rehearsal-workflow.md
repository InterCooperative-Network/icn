...
- **UI prototype** — Single guided shell (organizer mode) with the §3 screen list.
  - **Read-only preview slice (landed):** [#2237](https://github.com/InterCooperative-Network/icn/pull/2237) adds a fixture-only **Review Preview** tab to `web/pilot-ui/?mode=demo`. It renders the `preview-review` wrapper and canonical-matched pending-publish rows in plain language, including organizer-readable assignee labels, conditional review affordances, mutation consequences, and expected receipt categories. Every review choice is disabled; the surface records no decision, performs no mutation, creates no ActionCard, and issues no receipt/evidence. This advances steps 3–6 but does not complete the nine-step shell or the accessibility/organizer-rehearsal gates.
