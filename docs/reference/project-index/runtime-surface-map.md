...
- [`generated/icnctl-command-inventory.md`](generated/icnctl-command-inventory.md) — `icnctl` CLI command declarations from the clap tree, grouped by a curated role heuristic, uniform `unknown / needs local verification` status and `L1` proof (regenerate/check with `docs/scripts/icnctl_command_inventory.py`; issue [#2113](https://github.com/InterCooperative-Network/icn/issues/2113)). Declarations exist in source — not correctness, auth, live wiring, or organizer/production readiness.
