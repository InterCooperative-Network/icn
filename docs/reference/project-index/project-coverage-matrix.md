...
| In-repo institution package | source-tree map, package boundary docs | `institutions/`, `institutions/nycn/` | package-local | not public ICN core | package semantics do not belong in kernel/core | medium | source-tree map | Map against external NYCN repo. |
| External NYCN operator repo | NYCN README, validation scripts | `InterCooperative-Network/nycn` | package-local / rehearsal | organizer/operator-facing | private overlay and fixture-only boundaries | high: formal-pilot/private-data overclaim | source-tree map | Keep NYCN active-track language precise. |
