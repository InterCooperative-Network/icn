...
- **A deployment story beyond the frozen profile.** ADR-0086 (#2458) **is** adopted — `status: accepted` 2026-09-15, Alpha profile frozen at `860c6f6c22f18de0f7bc4cfb55e35b1143b3b4f1` — so you may state the decision plainly: appliance as the canonical sovereign node, Compose as a disposable devnet, Kubernetes/K3s as optional operator infrastructure, native Linux as an advanced install. What acceptance does **not** give you is any readiness claim: `implementation_status` is still *partially implemented*, independent restoration is still blocked on both its causes, and the profile means that exact revision rather than whatever `deploy/` currently holds. **Docker Compose is not sovereign-node proof**, and Kubernetes has **not** been retired from ICN (only the automatic private deployment from public CI was).
...
| **No readiness claim from the adopted deployment decision.** ADR-0086 **is** accepted (2026-09-15) and the Alpha profile is frozen at `860c6f6c22f18de0f7bc4cfb55e35b1143b3b4f1` — say so. Do **not** convert that into production readiness, a deployment authorization, or a closed restoration item; `implementation_status` is still *partially implemented*. | Truthfulness; the ADR's `status:` field owns adoption, and its `implementation_status` owns completeness — they are separate axes. |
