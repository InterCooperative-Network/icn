...
- **The evaluator package's name.** The lane shipped under "Common Sense (bootable) vertical slice" — a foreign identity that was **never an ICN-ratified name**. The correction to `icn-portable-evaluator` is PR #2435; **check `deploy/appliance/evaluator/package-spec.env` (`PKG_STEM`) for what the repository ships right now** rather than assuming either state. Never introduce the old name into new material. Never rename or delete released assets at or below 0.0.3 — they are retained so published checksums keep verifying, and the payloads under those names are genuine.
