...
| `institutions/nycn/` | The in-monorepo NYCN institution package (`bootstrap.yaml`, `charter/`, `config/`, `definitions/`, `seed/`, etc.). **Distinct from the separate operator repo at `InterCooperative-Network/nycn`** — see `pilot-and-nycn-map.md`. | `institutions/nycn/README.md` |
