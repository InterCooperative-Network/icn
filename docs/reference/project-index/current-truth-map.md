...
- **An adopted deployment decision** — ADR-0086 (PR #2458) is `proposed`, not adopted, and `implementation_status: partially implemented`. Only the appliance profile has a retained witness. **Independent appliance restoration is blocked**: `icnctl backup` omits `/etc/icn/icnd.env`, so a node restored from a backup alone cannot reopen its keystore.
- **Correct evaluator package identity** — `main` still ships `PKG_STEM="icn-common-sense-vertical-slice"`, a foreign name that was never an ICN identity. The rename to `icn-portable-evaluator` is unmerged (PR #2435). Releases at or below 0.0.3 are retained deliberately for checksum continuity.
