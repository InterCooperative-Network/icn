...
>
> One promise up front: this document describes a project that is **not finished and not in production use**. The [non-claims section](#non-claims) at the end says exactly what we are not claiming.
...
| Shop at REI | Consumer cooperative (roughly 24 million members) |
...
By International Cooperative Alliance estimates, there are roughly **3 million cooperatives worldwide** with around **1 billion members**, and the cooperative sector employs hundreds of millions of people globally. 2025 was the **UN International Year of Cooperatives** because the model matters.
...
- **Federations of cooperatives** (cooperatives of cooperatives — the Mondragon Corporation in Spain has tens of thousands of worker-owners across 100+ companies)
...
- **Not a bank, payment app, or accounting system.** Coops still need CPAs, banks, payment processors, accounting software. ICN sits beside those, holding the governance side of economic events (the decision to allocate; the authority that approved it), not the books themselves.
- **Not ready for production.** ICN is pre-pilot. The infrastructure runs in development and demo environments. The apps a cooperative member would actually use are still being built. This document is about what it's *trying to be*, not what's ready to use today.
...
1. **The cooperative movement is growing.** Worker cooperatives in the US have grown substantially over the last decade by movement census estimates. State and municipal governments are funding cooperative-development programs. The 2025 UN International Year of Cooperatives drew real attention.
...
- **Not production-ready.** ICN is not finished software. Nothing here is ready for a real cooperative to depend on.
- **Not in use.** No cooperative is running ICN in production. The "future cooperative running on ICN" in this document is exactly that: future.
- **Not a pilot.** No formal pilot exists.
- **Not handling anyone's private data.** ICN's demos and rehearsals use fictional or sanitized data only.
- **Not legal or financial advice.** Nothing here is a legal opinion about cooperatives, software, or anything else.
- **Not a promise.** This is one small team's attempt at a hard problem. It might fail. The [evidence map](ICN_INTRODUCTION_EVIDENCE_MAP.md) lists exactly what has been demonstrated so far, which is deliberately modest.
