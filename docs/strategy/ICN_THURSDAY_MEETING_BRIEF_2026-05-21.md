...
|---|---|---|---|
| **Core daemon / substrate (`icnd`)** | Tokio actor runtime, supervisor, kernel/app separation, gateway on port 8080, 44 workspace crates (37 in `crates/`, 4 in `apps/`, 3 in `bins/`). Identity (Ed25519 + Age keystore), networking (QUIC/TLS with DID-binding, mDNS), gossip with vector clocks and Bloom filters, double-entry mutual-credit ledger, CCL interpreter, governance proposal/vote primitives. | A daemon you build and run on your own hardware; identity, networking, ledger, governance primitives, and the gateway are working at the substrate level. Local + 3-node K3s convergence is tested. | "Production-ready." "Audited." "Scaling-tested past 10 nodes." "Hosted instance you can sign up for." |
