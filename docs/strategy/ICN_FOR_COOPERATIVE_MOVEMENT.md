...
>
> Every claim in this document is bounded by the [evidence map](ICN_INTRODUCTION_EVIDENCE_MAP.md) and the [non-claims section](#non-claims) at the end. If a sentence here sounds bigger than the evidence, the evidence map wins.
...
**What they do:** tokens, speculation, financialization of governance.
**Where ICN sits:** nowhere near them. ICN uses cryptographic verification for record integrity (signing decisions so you can verify them later), not for tokens, currency, or speculation. There is no consensus protocol, no mining, no global chain, no token. If anyone reads "peer-to-peer" or "cryptographic" and thinks Web3, they're reading the wrong project.
...
- **Not accounting software.** Not banking. Not financial-intermediary software in any form. ICN's vocabulary is governance-first by design: settlement, obligation, allocation, position, receipt, provenance, evidence.
- **Not production-ready.** Pre-pilot. The substrate runs in development and demo environments (daemon, gateway, identity, ledger primitives, governance primitives, federation primitives; first deployed to a [small self-hosted development cluster](../operations/deployment/HOMELAB_DEPLOYMENT.md) in December 2025). The human-facing surfaces — member-facing apps, steward dashboards, production federation flows — are next, not done. The strongest current proof is a local live-daemon receipt-chain demonstration and fixture-backed rehearsals; see the [evidence map](ICN_INTRODUCTION_EVIDENCE_MAP.md).
...
### "Who's funding it?"
Currently: self-funded and volunteer time. Grant funding from public-interest and movement-aligned technology funders is being pursued. Long-term: funding should come from movement institutions, not from VC. If you know movement-aligned funding sources, talk to us.
...
Fair. ICN is pre-pilot. We're not promising anything. The substrate runs in development and demo environments. The human-facing surfaces don't yet. If we disappear, the open-source substrate is on GitHub and can be forked. We are not trying to be the next Diaspora.

### "Will my cooperative have to learn cryptography?"
No. The cryptography is invisible plumbing for record integrity. A board member uses ICN the way they use email — they don't think about TLS. The whole point of the substrate is that the cooperative gets durable records without having to become a cryptography project.

### "What about privacy?"
Default privacy posture: nothing is public unless explicitly published. Member-private records stay on member-controlled hardware. Cross-coop verification works with hashes and signed claims, not raw data exposure. The privacy model has explicit primitives and an explicit threat model ([docs/security/threat-model.md](../security/threat-model.md)). What it does not have yet is production hardening — no cooperative should put real member data on ICN today, and ICN's own rehearsal practice uses fictional or sanitized data only.
...
A cooperative and solidarity economy that materially exists as a coordinated ecosystem — with governance, obligations, federation, and evidence — on infrastructure the movement owns and runs, instead of infrastructure rented from companies whose business models contradict ours. ICN is one bet at that infrastructure.
...
| **Settlement** | The final, durable record of an economic event. ICN's vocabulary choice for what others might loosely call "payment" (a deliberate regulatory boundary: ICN records settlement intent and evidence; it does not transmit money). |
...
| **Allocation** | An assignment of credits, patronage refunds, or other resources. |
| **Position** | A view of where a member or organization stands relative to credits, obligations, equity. |
...
| **Pre-pilot** | The current status of ICN. The substrate runs in development and demo environments. Human-facing surfaces don't yet. No live cooperative is running on ICN as production infrastructure. |
...
Not today (yet):
- Don't migrate your cooperative onto ICN. Pre-pilot means pre-pilot. The substrate runs in development environments; the surfaces a member would use don't exist yet.
...
To keep this document honest, here is what it does **not** claim:

- **Not production-ready.** No part of ICN is hardened for production use.
- **Not a pilot.** No formal pilot has been authorized with any cooperative.
- **Not live federation.** Multi-organization federation has not been demonstrated between real institutions.
- **Not private-data ready.** ICN has not been hardened for real member data. All rehearsals use fictional or sanitized data.
- **Not legal, privacy, or compliance advice.** Vocabulary discipline is a design posture, not a legal opinion. Regulatory questions belong to attorneys.
- **Not a claim that every spec is implemented.** Some of what this document describes is design intent. The [evidence map](ICN_INTRODUCTION_EVIDENCE_MAP.md) distinguishes what runs from what is designed.
- **Not adoption.** No cooperative is using ICN in production, and this document does not imply otherwise.
