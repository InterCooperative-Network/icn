...
- **Not production-ready.** Pre-pilot. The substrate runs in development and demo environments (daemon, gateway, identity, ledger primitives, governance primitives, federation primitives; first deployed to a small self-hosted cluster in December 2025). The human-facing surfaces — member-facing apps, steward dashboards, production federation flows — are next, not done. The strongest current proof is a local live-daemon receipt-chain demonstration and fixture-backed rehearsals; see the [evidence map](ICN_INTRODUCTION_EVIDENCE_MAP.md).
