...
STATUS_BY_COMMAND: dict[str, tuple[str, str]] = {
    ...
    "amendment submit": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/amendments/{id}/submit` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` AmendmentCommands::Submit); requires a running gateway, not integration-tested"),
    "amendment open-voting": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/amendments/{id}/vote` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` AmendmentCommands::OpenVoting); requires a running gateway, not integration-tested"),
    "amendment vote": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/amendments/{id}/ratify` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` AmendmentCommands::Vote); requires a running gateway, not integration-tested"),
    "amendment withdraw": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/amendments/{id}/withdraw` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` AmendmentCommands::Withdraw); requires a running gateway, not integration-tested"),
    "amendment add-change": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/amendments/{id}/changes` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` AmendmentCommands::AddChange); requires a running gateway, not integration-tested"),
    ...
    "appeal add-evidence": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/appeals/{id}/evidence` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` AppealCommands::AddEvidence); requires a running gateway, not integration-tested"),
    "appeal respond": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/appeals/{id}/respond` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` AppealCommands::Respond); requires a running gateway, not integration-tested"),
    "appeal withdraw": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/appeals/{id}/withdraw` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` AppealCommands::Withdraw); requires a running gateway, not integration-tested"),
