...
STATUS_BY_COMMAND: dict[str, tuple[str, str]] = {
    ...
    # live (pass 8, issue #2113) — local-only `charter` build/validate helpers and `steward`
    # config/topics readouts (no client/network in the arm; verified no reqwest/RPC).
    "charter create": (STATUS_LIVE, "local charter-document construction; builds the charter and `std::fs::write`s it to a file; no client/network (`icn/bins/icnctl/src/main.rs` CharterCommands::Create)"),
    "charter validate": (STATUS_LIVE, "local CCL validation `icn_ccl::schema::CclDocument::from_yaml` of a charter file; no client/network (`icn/bins/icnctl/src/main.rs` CharterCommands::Validate)"),
    ...
    # partial (pass 7, issue #2113) — `amendment` + `appeal` constitutional-governance gateway
    # HTTP clients. Every arm builds a `reqwest::Client` and hits a `/v1/constitutional/...`
    # route (all present in generated/route-inventory.md); depends on a running gateway, no
    # binary-spawning e2e test.
    "amendment propose": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/amendments` (route in `docs/reference/project-index/generated/route-inventory.md`; auth via get_gateway_token) (`icn/bins/icnctl/src/main.rs` AmendmentCommands::Propose); requires a running gateway, not integration-tested"),
    "amendment list": (STATUS_PARTIAL, "gateway client `reqwest GET {gateway}/v1/constitutional/amendments` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` AmendmentCommands::List); requires a running gateway, not integration-tested"),
    "amendment show": (STATUS_PARTIAL, "gateway client `reqwest GET {gateway}/v1/constitutional/amendments/{id}` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` AmendmentCommands::Show); requires a running gateway, not integration-tested"),
    "amendment submit": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/amendments/{id}/submit` (route in route-inventory; auth via get_gateway_token) (`icn/bins/icnctl/src/main.rs` AmendmentCommands::Submit); requires a running gateway, not integration-tested"),
    "amendment open-voting": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/amendments/{id}/vote` (route in route-inventory; auth via get_gateway_token) (`icn/bins/icnctl/src/main.rs` AmendmentCommands::OpenVoting); requires a running gateway, not integration-tested"),
    "amendment vote": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/amendments/{id}/ratify` (route in route-inventory; auth via get_gateway_token) (`icn/bins/icnctl/src/main.rs` AmendmentCommands::Vote); requires a running gateway, not integration-tested"),
    "amendment withdraw": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/amendments/{id}/withdraw` (route in route-inventory; auth via get_gateway_token) (`icn/bins/icnctl/src/main.rs` AmendmentCommands::Withdraw); requires a running gateway, not integration-tested"),
    "amendment add-change": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/amendments/{id}/changes` (route in route-inventory; auth via get_gateway_token) (`icn/bins/icnctl/src/main.rs` AmendmentCommands::AddChange); requires a running gateway, not integration-tested"),
    "appeal file": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/appeals` (route in route-inventory; auth via get_gateway_token) (`icn/bins/icnctl/src/main.rs` AppealCommands::File); requires a running gateway, not integration-tested"),
    "appeal list": (STATUS_PARTIAL, "gateway client `reqwest GET {gateway}/v1/constitutional/appeals` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` AppealCommands::List); requires a running gateway, not integration-tested"),
    "appeal show": (STATUS_PARTIAL, "gateway client `reqwest GET {gateway}/v1/constitutional/appeals/{id}` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` AppealCommands::Show); requires a running gateway, not integration-tested"),
    "appeal add-evidence": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/appeals/{id}/evidence` (route in route-inventory; auth via get_gateway_token) (`icn/bins/icnctl/src/main.rs` AppealCommands::AddEvidence); requires a running gateway, not integration-tested"),
    "appeal respond": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/appeals/{id}/respond` (route in route-inventory; auth via get_gateway_token) (`icn/bins/icnctl/src/main.rs` AppealCommands::Respond); requires a running gateway, not integration-tested"),
    "appeal withdraw": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/constitutional/appeals/{id}/withdraw` (route in route-inventory; auth via get_gateway_token) (`icn/bins/icnctl/src/main.rs` AppealCommands::Withdraw); requires a running gateway, not integration-tested"),
    # partial (pass 8, issue #2113) — `charter` gateway HTTP clients + `steward` daemon-RPC/gateway
    # clients (all routes present in generated/route-inventory.md). Each makes a real client call
    # but depends on a running daemon/gateway and has no binary-spawning e2e test.
    "charter show": (STATUS_PARTIAL, "gateway client `reqwest GET {gateway}/v1/charter/{id}` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` CharterCommands::Show); requires a running gateway, not integration-tested"),
    "charter list": (STATUS_PARTIAL, "gateway client `reqwest GET {gateway}/v1/charter` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` CharterCommands::List); requires a running gateway, not integration-tested"),
    "charter sign": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/charter/{id}/sign` (route in route-inventory; auth via get_gateway_token) (`icn/bins/icnctl/src/main.rs` CharterCommands::Sign); requires a running gateway, not integration-tested"),
    "charter ratify": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/charter/{id}/activate` (route in route-inventory; auth via get_gateway_token) (`icn/bins/icnctl/src/main.rs` CharterCommands::Ratify); requires a running gateway, not integration-tested"),
    "steward status": (STATUS_PARTIAL, "daemon RPC client `create_rpc_client` + `client.get_status()` (also reads local config.toml) in `handle_steward_command` (`icn/bins/icnctl/src/main.rs` StewardCommands::Status); requires a running daemon, not integration-tested"),
    "steward info": (STATUS_PARTIAL, "gateway client `reqwest GET` whose route branches on the input: `{gateway}/v1/steward/by-did/{steward}` if it starts with `did:`, else `{gateway}/v1/steward/{steward}` (both routes in route-inventory) (`icn/bins/icnctl/src/main.rs` StewardCommands::Info); requires a running gateway, not integration-tested"),
    "steward list": (STATUS_PARTIAL, "gateway client `reqwest GET {gateway}/v1/steward` (route in route-inventory) (`icn/bins/icnctl/src/main.rs` StewardCommands::List); requires a running gateway, not integration-tested"),
    ...
    # partial (pass 10, issue #2113) — `gov` domain/proposal/vote ops. `handle_gov_command` builds an
    # authenticated daemon RPC client (`create_authenticated_rpc_client` -> icn_rpc::RpcClient against a
    ...
    "gov domain add-member": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/gov/domains/{domain_id}/members` (bearer_auth; route `add_domain_member` @ `icn/apps/governance/src/http/configure.rs`, in route-inventory) — does not use the RPC client (`icn/bins/icnctl/src/main.rs` DomainCommands::AddMember); requires a running gateway, not integration-tested"),
    "gov proposal create": (STATUS_PARTIAL, "daemon RPC `client.call(\"governance.proposal.create\")` via `create_authenticated_rpc_client` (`icn/bins/icnctl/src/main.rs` ProposalCommands::Create); requires a running daemon, not integration-tested"),
    "gov proposal open": (STATUS_PARTIAL, "three daemon RPC calls via `create_authenticated_rpc_client`: `client.call(\"governance.proposal.get\")`, then `client.call(\"governance.domain.get\")`, then `client.call(\"governance.proposal.open\")` (`icn/bins/icnctl/src/main.rs` ProposalCommands::Open); requires a running daemon, not integration-tested"),
    "gov proposal list": (STATUS_PARTIAL, "daemon RPC `client.call(\"governance.proposal.list\")` via `create_authenticated_rpc_client` (`icn/bins/icnctl/src/main.rs` ProposalCommands::List); requires a running daemon, not integration-tested"),
    ...
    "gov proposal cancel": (STATUS_PLANNED, "`bail!(\"Cancel command not yet supported via RPC. Use 'proposal close' instead, or stop the daemon and modify the store directly.\")` before any client call (`icn/bins/icnctl/src/main.rs` ProposalCommands::Cancel)"),
    "gov vote show": (STATUS_PLANNED, "`bail!(\"Vote show command not yet supported via RPC. Use 'proposal show {proposal_id}' to see the proposal.\")` before any client call (`icn/bins/icnctl/src/main.rs` VoteCommands::Show)"),
    # live (pass 11, issue #2113) — `federation` peer/config arms are pure local `icn.toml` reads/writes
    # (`Config::from_file` / `config.to_file` on data_dir/icn.toml in `handle_federation_command`); no
    # network call, no daemon/gateway dependency — same local-only criterion as `steward config`.
    "federation add": (STATUS_LIVE, "local-only: validates the `icn://` URL then appends the peer to `network.bootstrap_peers` and `config.to_file(icn.toml)` — no client/network (`icn/bins/icnctl/src/main.rs` FederationCommands::Add)"),
    "federation remove": (STATUS_LIVE, "local-only: removes matching bootstrap peers from `icn.toml` and `config.to_file` — no client/network (`icn/bins/icnctl/src/main.rs` FederationCommands::Remove)"),
    "federation config": (STATUS_LIVE, "local-only: reads `icn.toml` and prints the `[federation]` config — no client/network (`icn/bins/icnctl/src/main.rs` FederationCommands::Config)"),
    "federation set": (STATUS_LIVE, "local-only: sets a `federation.*` key in `icn.toml` and `config.to_file` — no client/network (`icn/bins/icnctl/src/main.rs` FederationCommands::Set)"),
    # partial (pass 11, issue #2113) — `federation` arms with a REAL, server-WIRED client path: daemon RPC
    # via `icn_rpc::RpcClient` (each cited `network.*`/`federation.*` method is registered in the daemon
    # dispatch `icn/crates/icn-rpc/src/server.rs`) or a gateway HTTP route present in route-inventory.md.
    # Daemon/gateway-dependent, no binary-spawning e2e test (no icnctl test references `federation`).
    "federation status": (STATUS_PARTIAL, "daemon RPC `client.get_peers()` -> `network.peers` (registered `icn/crates/icn-rpc/src/server.rs`:639); prints config-only status if the daemon is down (`icn/bins/icnctl/src/main.rs` FederationCommands::Status); requires a running daemon, not integration-tested"),
    "federation peers": (STATUS_PARTIAL, "lists `icn.toml` bootstrap peers, then annotates connection state via daemon RPC `client.get_peers()` -> `network.peers` (server.rs:639, best-effort) (`icn/bins/icnctl/src/main.rs` FederationCommands::Peers); requires a running daemon for status, not integration-tested"),
    "federation connect": (STATUS_PARTIAL, "daemon RPC `client.dial(did, addr)` -> `network.dial` (registered server.rs:640) (`icn/bins/icnctl/src/main.rs` FederationCommands::Connect); requires a running daemon, not integration-tested"),
    "federation invite": (STATUS_PARTIAL, "derives DID from the local keystore, then best-effort daemon RPC `client.get_status()` -> `network.status` (registered server.rs:642) for the listen address (`icn/bins/icnctl/src/main.rs` FederationCommands::Invite); requires a running daemon for the full invite URL, not integration-tested"),
    "federation gateway-connect": (STATUS_PARTIAL, "gateway client `reqwest POST {gateway}/v1/federation/connect` (bearer_auth; route `federation_connect` @ `icn/crates/icn-gateway/src/api/federation.rs`, in route-inventory) (`icn/bins/icnctl/src/main.rs` FederationCommands::GatewayConnect); requires a running gateway, not integration-tested"),
    "federation coop list": (STATUS_PARTIAL, "daemon RPC `client.federation_coop_list()` -> `federation.coop.list` (registered server.rs:852) (`icn/bins/icnctl/src/main.rs` CoopCommands::List); requires a running daemon, not integration-tested"),
    "federation coop show": (STATUS_PARTIAL, "daemon RPC `client.federation_coop_get()` -> `federation.coop.get` (registered server.rs:855) (`icn/bins/icnctl/src/main.rs` CoopCommands::Show); requires a running daemon, not integration-tested"),
    "federation coop register": (STATUS_PARTIAL, "daemon RPC `client.federation_coop_register()` -> `federation.coop.register` (registered server.rs:858) (`icn/bins/icnctl/src/main.rs` CoopCommands::Register); requires a running daemon, not integration-tested"),
    "federation coop update": (STATUS_PARTIAL, "daemon RPC `client.federation_own_update()` -> `federation.own.update` (registered server.rs:865) (`icn/bins/icnctl/src/main.rs` CoopCommands::Update); requires a running daemon, not integration-tested"),
    "federation vouch issue": (STATUS_PARTIAL, "daemon RPC `client.federation_vouch_issue()` -> `federation.vouch.issue` (registered server.rs:871) (`icn/bins/icnctl/src/main.rs` VouchCommands::Issue); requires a running daemon, not integration-tested"),
    "federation vouch list": (STATUS_PARTIAL, "daemon RPC `client.federation_own_get()` -> `federation.own.get` (server.rs:864) + `federation_coop_list()` -> `federation.coop.list` (852) + `federation_vouch_list()` -> `federation.vouch.list` (registered server.rs:868) (`icn/bins/icnctl/src/main.rs` VouchCommands::List); requires a running daemon, not integration-tested"),
    "federation vouch revoke": (STATUS_PARTIAL, "daemon RPC `client.federation_vouch_remove()` -> `federation.vouch.remove` (registered server.rs:875) (`icn/bins/icnctl/src/main.rs` VouchCommands::Revoke); requires a running daemon, not integration-tested"),
    "federation attestation list": (STATUS_PARTIAL, "daemon RPC `client.federation_attestation_list()` -> `federation.attestation.list` (registered server.rs:880) (`icn/bins/icnctl/src/main.rs` AttestationCommands::List); requires a running daemon, not integration-tested"),
    "federation attestation from": (STATUS_PARTIAL, "daemon RPC `client.federation_attestation_from()` -> `federation.attestation.from` (registered server.rs:884) (`icn/bins/icnctl/src/main.rs` AttestationCommands::From); requires a running daemon, not integration-tested"),
    "federation attestation issue": (STATUS_PARTIAL, "daemon RPC `client.federation_attestation_issue()` -> `federation.attestation.issue` (registered server.rs:888) (`icn/bins/icnctl/src/main.rs` AttestationCommands::Issue); requires a running daemon, not integration-tested"),
    "federation clearing list": (STATUS_PARTIAL, "daemon RPC `client.federation_clearing_list()` -> `federation.clearing.list` (registered server.rs:899) (`icn/bins/icnctl/src/main.rs` ClearingCommands::List); requires a running daemon, not integration-tested"),
    "federation clearing show": (STATUS_PARTIAL, "daemon RPC `client.federation_clearing_show()` -> `federation.clearing.show` (registered server.rs:902) (`icn/bins/icnctl/src/main.rs` ClearingCommands::Show); requires a running daemon, not integration-tested"),
    "federation clearing create": (STATUS_PARTIAL, "daemon RPC `client.federation_clearing_create()` -> `federation.clearing.create` (registered server.rs:905) (`icn/bins/icnctl/src/main.rs` ClearingCommands::Create); requires a running daemon, not integration-tested"),
    "federation clearing position": (STATUS_PARTIAL, "daemon RPC `client.federation_clearing_position()` -> `federation.clearing.position` (registered server.rs:914) (`icn/bins/icnctl/src/main.rs` ClearingCommands::Position); requires a running daemon, not integration-tested"),
    "federation clearing settle": (STATUS_PARTIAL, "daemon RPC `client.federation_clearing_settle()` -> `federation.clearing.settle` (registered server.rs:918) (`icn/bins/icnctl/src/main.rs` ClearingCommands::Settle); requires a running daemon, not integration-tested"),
