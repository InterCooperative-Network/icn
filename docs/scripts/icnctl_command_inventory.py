...
STATUS_BY_COMMAND: dict[str, tuple[str, str]] = {
    ...
    "network status": (STATUS_PARTIAL, "daemon RPC client `client.get_nat_status()` via `create_rpc_client` (`icn/bins/icnctl/src/main.rs` NetworkCommands::Status); requires a running daemon, not integration-tested"),
