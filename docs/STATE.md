...
**LAN workstation witness (2026-07-14):** the appliance's new LAN single-origin profile (this branch, image commit `916629d7`) was deployed as a dedicated hypervisor VM on operator-controlled LAN infrastructure and the full organizer→member loop was driven twice from a real Windows workstation browser — one-click role-scoped sessions (no terminal, no credential paste, no credential in any URL), digest-bound confirm with fail-closed stale-preview handling, fresh member session, durable completion receipt, in-VM steward verify PASS, unattended recovery across service restart and VM reboot. A LAN development rehearsal, not production and not any human gate — see the 2026-07-14 sync block above.
