...
**Deployment profiles: ACCEPTED 2026-09-15, profile frozen.** ADR-0086 is **on `main`** (merged `9ca12148`, #2458) and linked from `docs/INDEX.md`. It records the four-profile direction — appliance = canonical sovereign node; Compose = disposable devnet; Kubernetes/K3s = optional operator infrastructure; native Linux = advanced install — at `status: accepted` (2026-09-15), `implementation_status: partially implemented` — separate axes. The Alpha profile is frozen at `860c6f6c22f18de0f7bc4cfb55e35b1143b3b4f1`. **Writing or merging that ADR did not adopt it**; adoption was a human decision and the `status:` field in `docs/adr/` is the owner of that fact. Acceptance settles the classification and the frozen revision; it certifies no production readiness and closes no open item. Only the appliance profile has a retained build-and-boot witness, and independent appliance restoration is **blocked** for two independent reasons, and closing either alone does not unblock it: `icnctl backup` omits `/etc/icn/icnd.env`, so a node restored from a backup alone cannot reopen its keystore; and a restored ledger cannot be shown to be *complete*, because a truncated journal reopens as a valid short prefix and nothing commits to the extent it should have had (#2746; the independent evidence this needs is owned by #2786, where the mechanism is not yet selected).
