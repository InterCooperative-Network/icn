...
| [`./pending-publish-summary.schema.json`](pending-publish-summary.schema.json) | `urn:icn:contract:pending-publish-summary:v1` | non-DNS URN | **keep** | Authored on the preferred non-DNS pattern (icn#1728). Same companion-field convention (`x-icn-contract-name`, `x-icn-distribution-hints`) plus an `x-icn-composes-with` field pointing at `urn:icn:contract:preview-review:v1`. Full rationale in [`./pending-publish-summary.md`](pending-publish-summary.md) Identity section. Read-only row-level read-model composed inside the preview-review boundary; explicitly not a mutation API. |
...
- **Keep**: `preview-review.schema.json` and `pending-publish-summary.schema.json` (both on the preferred non-DNS pattern; the latter composes with the former via `x-icn-composes-with`).
