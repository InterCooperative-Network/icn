...
**Why the hash-bind invariant matters**: `OPAQUE_REC_PREFIX`'s write-once-by-hash check already rejects **different payload bytes** under the same `(class, record_hash)` primary key, so the hash-bind keyspace is **not** the body-uniqueness guard. It closes a different failure mode: replaying the **same** `(class, record_hash, payload)` tuple under a **different** `(key1, key2_opt, recorded_at)` secondary-index tuple. Without `OPAQUE_HASH_BIND_PREFIX`, that replay would let one canonical body fan out across multiple audit chains or surface at a later timestamp under `get_latest_opaque`. The hash-bind keyspace closes that hole atomically: the transaction either commits the first index-tuple binding or aborts the second; a `(class, record_hash)` that has been bound to one `(key1, key2_opt, recorded_at)` tuple stays bound to that tuple forever.
