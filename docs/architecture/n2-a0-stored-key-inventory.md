...
| 25 | icn-gateway | `receipt_store.rs:1090 grant_by_grantee_key` | sled | authority grants by grantee | `pfx ‖ len(tag ‖ did.as_str()) ‖ valid_from ‖ grant id` (ADR-0014 layout, §10.4) | D | `from_str` | none | live | **SILENT-MERGE RISK** — *2026-09-04:* dispositioned as a **derived projection** of the canonical `adr0014:grant:<uuid>` records (migration gate §11.6, #2627 M2): a Person query decodes every Person-tagged spelling to its principal, proves each candidate against the primary `AuthorityGrant` before returning it, and de-duplicates by `AuthorityGrantId` so two spellings of one grant are one grant and two grant ids stay two grants; Entity-tagged rows keep exact-string identity and are never principalized; malformed projection rows refuse rather than vanish. Registered in `n2a_keyspaces` as `icn-gateway/adr0014_grant_by_grantee` under a third structural descriptor, `LengthPrefixedTagged { principal_tag: 0x01 }` (a big-endian `u32` frame) — the two existing regions cannot read a binary key — `Equivalent`/`Established`, consumed by the #2700 startup gate, which one ordinary Person grant previously made refuse. No byte re-keyed |
| 26 | icn-gateway | `listings_mgr.rs interest_index_key` (writer: `add_interest_if_not_duplicate`; maintenance: `cleanup_orphaned_interest_indexes`) | sled | interest by (listing, from) | `pfx ‖ listing ‖ from_did` — value is a constant sentinel `"1"`, **never** the interest id, so the canonical row cannot be resolved from this row by lookup | D | `from_str` (the JWT `sub`, rendered back through `Display`) | none — but it is **not a lookup projection**: the sled `compare_and_swap` on this key *is* the one-interest-per-member rule, so a second spelling is a second permission to write a canonical `v1:interest:` row | live | **SILENT-MERGE RISK** — *2026-09-05:* dispositioned as a **uniqueness lock over** the canonical `v1:interest:` rows (migration gate §11.9, #2627 M4b): **COVERED**. Registered in `n2a_keyspaces` as `icn-gateway/listing_interest_uniqueness`, prefix `v1:interest_idx:`, `PrincipalRegion::WholeKey` with `did_ends_key`, `FailClosed` / `Established`, the sixteenth whole-key layout, consumed by the #2700 startup gate. Collision unit is the pair **(listing, Principal)** — the listing `Uuid` stays in the canonical shape, so one member on two listings is two shapes and not a group. The live writer decides duplicates against the canonical rows under `Did` equality with classification and write serialised, and refuses unreadable evidence rather than reading it as absence; the CAS is retained unchanged as the same-spelling decider. The maintenance parser followed the writer's framing only after M4b — splitting the whole key on `':'` counted the DID's own colons and skipped every production row, so the pass was a no-op and an orphaned lock could refuse its principal indefinitely. No byte re-keyed. The **canonical** `v1:interest:<listing>:<uuid>` rows carry their principal in the *value* and remain outside the key collision scanner: runtime-guarded, scanner-blind, migration unresolved. Already-persisted duplicate interests are not reconciled and no survivor is chosen |
...
| 65 | icn-commons | `inner.rs:346–362 update_display_name` | `Store` | weak-holder record (minted on first display-name update) | `"commons/holders/" ‖ hex(SHA-256(did.to_string()))` + a by_did row | D | path `{did}` (`api/members.rs:141`) gated by `caller_did != did` and `coop.members.iter().any(\|m\| m.did == did)` — **`Did` equality** | the gate itself: after I7 the `Did`-equality gate passes for an aliased spelling while the spelling-hashed store misses ⇒ a **second** `CommonsHolderRecord` is minted for one key | live | **NEEDS MIGRATION** (§3.1 clause (d) — I7 makes the authorization equality and the storage key disagree) — **2026-09-05 (#2627 M3): guarded before mint.** `update_display_name` no longer reads an exact-key miss as principal absence: it classifies the holder-by-DID namespace for the requested Principal first (`icn_commons::store::classify_holder_mint`) and refuses when a row under any spelling already names that Principal, when an index row's primary cannot be proven, when index and primary disagree, or when a row cannot be read at all. The mint itself is byte-for-byte unchanged — a proven-absent spelling still derives `holder_id = anchor_id = SHA-256(did.to_string())` at `Weak` — so no persisted holder identity moves. The spelling-derived id remains the P2 migration prerequisite of §5 decision A; M3 prevents *new* alias duplication and disposes of no existing duplicate |
| 66 | icn-commons | `store.rs:39 ANCHOR_BY_DID_PREFIX` (`:655 put_anchor`, `:717 put_anchor_did_index`, `:705 get_anchor_by_did`, `:849 delete_anchor`) | `Store` | anchor id by DID | `"commons/anchors/by_did/" ‖ <spelling>` — **two writers, two row families per anchor**: `put_anchor` files it under `anchor.to_did()` and `put_anchor_did_index` files the *same* anchor under the enrollment spelling, which appears nowhere in the anchor body | D | the derived family is `Anchor::to_did()` (~50% unreadable as a validated `Did`, §10.1 — so this namespace must be grouped by `identifier_bytes_of_spelling`, never by `Did::from_str`); the enrollment family is a wire-parsed `Did`; read exact-spelling (`commons_mgr.rs:113`, `api/commons/mod.rs:131`); `delete_anchor` removes only the derived family, orphaning the enrollment row | none (`LruCache<String,_>` keyed by hex id) | live | **SILENT-MERGE RISK** — 2026-09-05 (#2627 M4a): **COVERED**. Registered `icn-commons/anchor_by_did`, `FailClosed` / `Established`, and the enrollment constructor now classifies the principal against the whole namespace before the first durable write (`classify_anchor_enrollment`, gate §11.8). Two rows per healthy anchor are *not* a collision — they name two different principals. Already-derived duplicate anchors, the dangling-row leak in `delete_anchor` and the three-write crash window are not dispositioned. |
| 67 | icn-commons | `store.rs:39 HOLDER_BY_DID_PREFIX` (`:608, :662, :691`) | `Store` | holder id by DID | `"commons/holders/by_did/" ‖ holder_did.to_string()` | D | wire `from_str` (`inner.rs:301 create_holder_from_anchor`) | none — but it is an **authorization gate**: `icn-gateway authority.rs:44, 89 require_office_in_jurisdiction`, `api/membership/mod.rs:152, 195, 719`, `api/steward/mod.rs:517` do `get_holder_by_did(caller_did)` → not-found ⇒ `AuthorizationFailed` | live | **SILENT-MERGE RISK** (fail-closed split) — **2026-09-05 (#2627 M3): registered `FailClosed`/`Established`** in `n2a_keyspaces` as `icn-commons/holder_by_did`, prefix `commons/holders/by_did/`, `PrincipalRegion::WholeKey` with `did_ends_key`, the fourteenth whole-key layout. Two spellings of one Principal reach two independent `CommonsHolderRecord`s with their own status, personhood level and rights, and choosing a survivor is a decision about a member's standing that no identity-layer rule makes; two rows naming one holder id are refused on the same ground. The sibling subspaces `commons/holders/<hex holder id>` and `commons/holders/by_anchor/<hex anchor id>` are keyed by opaque hex, lie outside the registered prefix and are neither claimed nor cleared by it. Consumed by the #2700 startup gate, which one ordinary weak-holder row previously made refuse as uncovered. The authorization split of this index is unchanged: `get_holder_by_did` stays fail-closed per spelling and is not made alias-transparent |
...
| 81 | apps/governance | `manager.rs assignee_idx_key`, reader `list_by_assignee` | sled | action items by assignee | `"action_item_by_assignee:" ‖ did.as_str() ‖ ':' ‖ domain ‖ ':' ‖ item` — value is a constant sentinel `"1"` | D | `ActionItem.assignee: Option<Did>` (`icn-governance action_item.rs`) from HTTP/governance wire `from_str` | `list_by_assignee` decodes every anchored spelling to its principal and proves each candidate against the canonical `action_item:` row | live | ~~**SILENT-MERGE RISK**~~ — *2026-09-06:* dispositioned as a **derived lookup projection** over the canonical `action_item:<domain>:<item>` rows (migration gate §11.10, #2627 M4c): **COVERED**. Registered in `n2a_keyspaces` as `icn-governance-actor/action_item_by_assignee`, prefix `action_item_by_assignee:`, `PrincipalRegion::AnchoredThenOpaque { terminator: b':' }`, `Equivalent` / `Established`, the third anchored layout and the first whose terminator is `':'`, consumed by the #2700 startup gate. Collision unit is **(assignee Principal, exact domain id, exact ActionItemId)** — domain and item ids stay in the canonical shape, so one person holding many action items is many shapes and never a group. The I7 regression is fixed at both ends: `save` decided stale-row removal with `existing.assignee != item.assignee`, which is principal equality since I7 and therefore false for two spellings of one person, so a spelling-only reassignment through `PUT /gov/domains/{d}/action-items/{i}` leaked the old row; and the reader scanned the querying spelling as an exact byte prefix, so `/gov/me/work` and `/gov/digest` missed items assigned under another spelling of the caller. The writer now compares the exact projection **keys**; the reader is principal-aware and de-duplicates by canonical identity. Delete and `delete_all` retire every spelling; `rebuild_assignee_index` recomputes the projection from canonical rows on explicit call, never at startup. No byte re-keyed, no spelling normalized, no preferred encoding. Already-persisted historical alias rows are not reconciled and no survivor is chosen |
