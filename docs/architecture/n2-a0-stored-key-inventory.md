...
| 27 | icn-federation | `attestation_store.rs:38 attestation_key` | `Store` | (member, source coop) attestation | `pfx ‖ did.as_str() ‖ '/' ‖ coop_id` | D | `from_str` | #59 `LruCache<Did,_>` read-through (`:80–113`) — a per-lookup cache, not a start-up rebuild | live | **SILENT-MERGE RISK** (re-key the cache in N2-A, #59) |
