...
| 25 | icn-gateway | `receipt_store.rs:1090 grant_by_grantee_key` | sled | authority grants by grantee | `pfx ‖ len(tag ‖ did.as_str()) ‖ …` (ADR-0014 layout, §10.4) | D | `from_str` | none | live | **SILENT-MERGE RISK** |
...
| 27 | icn-federation | `attestation_store.rs:38 attestation_key` | `Store` | (member, source coop) attestation | `pfx ‖ did.as_str() ‖ '/' ‖ coop_id` | D | `from_str` | #59 `LruCache<Did,_>` read-through (`:80–113`) — a per-lookup cache, not a start-up rebuild | live | **SILENT-MERGE RISK** (re-key the cache in N2-A, #59) — *2026-09-03:* the cache is gone and every read classifies the namespace by `Did` equality, with a same-source alias pair refused rather than merged (#2704, #2703); registered in `n2a_keyspaces` as `icn-federation/attestations`, fail closed |
| 28 | icn-federation | `agreement/store.rs:86 party_index_key` | `Store` | agreements by party | `pfx ‖ did.as_str() ‖ '/' ‖ agreement` | D | `from_str` | none | live | **SILENT-MERGE RISK** |
...
| 70 | icn-ledger | `treasury/audit.rs:289, 317` | `Store` | treasury audit trail | `"ledger:treasury:audit:" ‖ Display ‖ ':' ‖ ts ‖ ':' ‖ id` (read by `scan_reverse_paginated` on the `Display` prefix) | D | `record.treasury_did: Did` | none | live | **SILENT-MERGE RISK** (audit history split across spellings) |
...
| 41 | icn-ledger | `treasury.rs:215–254` five `HashMap<Did,_>` | B | rows #10 + budgets/rules/shares (`:1021 insert` on load) | N treasuries → 1 — but no wire spelling reaches #10's writer today | live | **SILENT-MERGE RISK** (consumer of #10; see its flag) |
