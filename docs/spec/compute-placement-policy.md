...
- **Workload kind.** One of CCL / WASM / SQL-reference / model-or-advisory / transform / verification, per the closed runtime-class taxonomy in `docs/spec/governed-service-binding.md` §"Runtime classes."
- **Privacy class.** Per ADR-0030 and `icn-compute/src/types.rs` (current variants `Public`, `Member`, `NeedToKnow`). Forward-direction richer taxonomies are tracked in `#1792`; the spec does not pre-decide which taxonomy will land first.
...
The placement decision produces exactly one of:
...
- **`ExecutorAdmissionDecision`** — a sub-result emitted by the executor (or commons pool) after the placement decision has selected it. Records whether the executor admitted the workload, whether the admission is conditional (e.g., must wait for capacity), and any executor-side constraints (timeouts, retry limits).
- **`PlacementFallbackReceipt`** — recorded when a placement falls back from a preferred class to a lower-priority class (e.g., `DomainLocalPreferred` falls back to `LocalDomainBound`). Names the original placement class, the fallback placement class, the reason, the authority basis for the fallback.
- **`ReviewRequiredActionCard`** — when the workload is advisory and the policy requires human ratification before its output is treated as institutional state. The action card surfaces in the operator dashboard or member shell per `#1818` / `#1795`.
...
8. **Package-specific nouns stay in packages.** No placement core uses cooperative / federation / community / NYCN / partner-specific nouns as generic primitives. Per `docs/architecture/INSTITUTION_PACKAGE_BOUNDARY.md` §C3, the structural names are `LocalDomain`, `InstitutionalDomain`, `Domain`, `DomainPolicy`. Institution packages bring their own role identifiers and ceremony markers.
...
Placement decisions emit receipts via the receipt envelope defined in ADR-0026 and the effect dispatch chain in `docs/spec/effect-dispatch-contract.md`. This spec does not add new receipt classes. The receipts placement work touches:

- **`PlacementDecision`** is recorded as a Stage 5 evidence artifact (per `docs/spec/effect-dispatch-contract.md` §"Stage 5 — Application and evidence"). It is not authoritative state by itself; the authoritative effect comes from execution.
- **`PlacementRejected`** is recorded as a Stage 5 evidence artifact. It does not produce an `InstitutionalEffectRecord`; it produces an `EffectDispatchEvidence` capturing the rejection.
- **`PlacementFallbackReceipt`** is recorded as a Stage 5 evidence artifact attached to the resulting `PlacementDecision`.
