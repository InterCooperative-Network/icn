...
- Implementation of the remaining design-level artifact names. `RoutingProof` and `RedundancyProof` remain forward-direction names. The receipt-index anti-entropy fixture loop (Slice A) is tracked under `#1838`. The follow-ups that retrofit the cockpit / member-shell rendering fixtures to consume `SyncDegradedStatus` for the "Sync delayed" / "Action paused until records sync" / "Sync delayed / degraded" surfaces are intentionally split out of `#1856` and remain forward work. Slice C (`QuorumSyncCheck` fixture federation placement) — which exercises the federation-scope schema landed by `#1860` end-to-end — also remains forward work.
...
None of these wire shapes mutates protocol topics, emits probes, classifies divergences live, applies repairs, or adds a new ADR-0026 receipt class — the kernel records are evidence envelopes that travel inside an existing Stage 5 `EffectDispatchEvidence` per `docs/spec/effect-dispatch-contract.md` (or, for blob-transfer repairs, alongside a Layer 2 `ArtifactReceipt` per ADR-0026). The remaining identifiers below remain design-level names; `RoutingProof` and `RedundancyProof` are forward-direction and not yet tracked under a dedicated issue. The fixture loop is tracked under `#1838`, the retrofit that wires the `FixtureSyncOutcome` stand-in in the receipt-index fixture to consume the public `PeerSyncReport` is tracked under #1854 (PR #1855), and the retrofit that wires the cockpit / member-shell rendering surfaces to consume the public `SyncDegradedStatus` is tracked under #1858 (PR #1859). Slice C (`QuorumSyncCheck` fixture federation placement) is the next fixture unit and remains a separate forward unit.
