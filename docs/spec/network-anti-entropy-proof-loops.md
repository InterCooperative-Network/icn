...
- Implementation of the remaining design-level artifact names. `QuorumSyncCheck`, `FederationSyncWindow`, `RoutingProof`, and `RedundancyProof` remain forward-direction names. The receipt-index anti-entropy fixture loop (Slice A) is tracked under `#1838`. The follow-ups that retrofit the cockpit / member-shell rendering fixtures to consume `SyncDegradedStatus` for the "Sync delayed" / "Action paused until records sync" / "Sync delayed / degraded" surfaces are intentionally split out of `#1856` and remain forward work.
