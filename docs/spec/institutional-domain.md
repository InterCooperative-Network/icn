...
> **Status (#2142).** The persisted, gated domain-policy adoption **HTTP route has landed** (#2172), and **declaration is now mandate-gated at the manager seam** (#2174). #2142 remains open: these are one adoption surface plus a gated declare seam, not the full lifecycle, and **no declare/create HTTP route exists yet**.
...
**Still open / explicitly not landed.** **No declare/create HTTP route exists yet:** declaration is now mandate-gated at the manager seam (`declare_institutional_domain_gated`, #2174), which resolves `MandateAct::DeclareInstitutionalDomain` through `DefaultMandateGate` before any store write — the flagged ADR-0083 sub-question (declaration is an authority-bearing act) is settled in favor of gating. The remaining rung is the thin declare HTTP route, which must call the **gated** seam (never the ungated bootstrap one). Also forward work: the full `InstitutionalDomain` / `DomainPolicy` lifecycle (standing, services, routing, federation, exit); durable `DomainPolicy` **body** storage and the CCL policy registry / evaluator selection / CCL evaluation (#1817); the service-binding runtime; package activation; and cross-process transactional adoption safety. **#2142 remains open.**
