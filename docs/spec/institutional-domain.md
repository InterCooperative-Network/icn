...
| Minimal runtime root for `InstitutionalDomain` / `DomainPolicy` (Declare + Adopt slice) | `ADR-0083` (#2142). Declare + Adopt-policy slice **landed** in `icn-governance` (`institutional_domain` module: `InstitutionalDomain`, `DomainPolicy` / `DomainPolicyRef`, fail-closed `adopt_policy`; tests cover declare, adopt, structural inertness, and missing/ambiguous/unbound/inactive authority). App-side adoption is now **gate-resolved**: `apps/governance::domain_policy_adoption::adopt_domain_policy_gated` runs the existing `DefaultMandateGate::require()` (actor → active grants → `TypedScope.domain` + Execution class + `domain_policy:adopt` act token + mandate lifecycle) before the pure-core structural commit (#2142 follow-up). The adoption seam is now reachable at the governance app boundary via `GovernanceManager::adopt_domain_policy` (#2166), persisted via `GovernanceManager::adopt_domain_policy_persisted` over the `InstitutionalDomain` store (#2170), and **exposed over HTTP** at `POST /gov/domains/{domain_id}/domain-policy/adopt` (#2172). Declaration is now **mandate-gated at the manager seam** via `GovernanceManager::declare_institutional_domain_gated` (#2174), resolving `MandateAct::DeclareInstitutionalDomain` through `DefaultMandateGate` before any store write; the ungated `declare_institutional_domain` remains a bootstrap/in-process-only seam. The gated declaration is now **exposed over HTTP** at `POST /gov/domains/{domain_id}/institutional-domain/declare` (#2176). Full lifecycle (standing, services, routing, federation, exit) and CCL evaluation remain forward work — see "InstitutionalDomain declaration and domain-policy adoption: app boundary and HTTP routes" below. |
...
## InstitutionalDomain declaration and domain-policy adoption: app boundary and HTTP routes

> **Status (#2142 closed-completed; #2178 body store).** Both gated HTTP routes landed under #2142: domain-policy **adoption** (`POST .../domain-policy/adopt`, #2172) and `InstitutionalDomain` **declaration** (`POST .../institutional-domain/declare`, #2176), the latter over the mandate-gated manager seam from #2174; #2142 was closed-completed on 2026-06-23 after acceptance review. The first post-#2142 rung (#2178) adds a durable `DomainPolicy` **body store** seam (below): raw policy bytes keyed by their content-addressed `DomainPolicyId`, re-hashed on read/write and failing closed on key/body mismatch. The adoption route is **not yet wired** to persist bodies, and the full CCL policy registry / evaluator selection / CCL evaluation (#1817) remains forward work, as does the full `InstitutionalDomain` / `DomainPolicy` lifecycle (standing, services, routing, federation, exit).

The #2142 lane built the adoption path bottom-up, each rung a separate reviewable PR:
...
- **Persistence seam (#2170), per the ADR-0083 persistence addendum (2026-06-23).** `GovernanceStateStore` gains default-implemented, fail-closed `get_institutional_domain` / `save_institutional_domain`; `SledGovernanceStateStore` persists the record under a distinct `gov:institutional-domain:` key space (sibling to `gov:domain:`), storing only the adopted `current_policy: DomainPolicyRef` (no `DomainPolicy` body — #1817). `GovernanceManager::adopt_domain_policy_persisted` does `load → gate → adopt → save`, leaving state unchanged on rejection. This resolved the ADR-0083 open questions a store must commit to: key on the existing `GovernanceDomainId` (Q1 — no parallel id), a **separate sibling record** (Q2 — no `GovernanceDomain` consolidation), and store only the adopted `current_policy` pointer (Q4 — `DomainPolicy` body deferred to #1817).
- **HTTP route (#2172).** `POST /gov/domains/{domain_id}/domain-policy/adopt`.
- **Gated declaration seam (#2174).** `GovernanceManager::declare_institutional_domain_gated` — resolves `MandateAct::DeclareInstitutionalDomain` (Execution class; act token `institutional_domain:declare`) through `DefaultMandateGate` **before** any store write, then delegates to the ungated `declare_institutional_domain`. Fails closed: `MissingReceiptBackend` (no backend), `Unauthorized(MandateRejection)` (a 403-class gate denial), `Backend(String)` (a 5xx-class gate read failure — kept distinct from a denial), and `Store(...)` (no domain store, already declared, or a write error). The ungated `declare_institutional_domain` is retained as a documented bootstrap/in-process-only seam that must never be wired to a routable surface.
- **Declaration HTTP route (#2176).** `POST /gov/domains/{domain_id}/institutional-domain/declare`.
- **DomainPolicy body store (#2178), first rung past ADR-0083 Q4.** `GovernanceStateStore` gains default-implemented, fail-closed `get_domain_policy_body` / `save_domain_policy_body`; `SledGovernanceStateStore` overrides both under a distinct content-addressed `gov:domain-policy-body:{DomainPolicyId}` key space (sibling to `gov:institutional-domain:`), storing the raw policy body bytes whose blake3 hash is the key. Both directions re-hash with `DomainPolicyId::from_content` and **fail closed** on key/body mismatch, so a body can never be filed under — or returned for — a key it does not hash to; identical content dedupes by construction. Where #2170 stored only the adopted `current_policy: DomainPolicyRef` pointer (Q4 deferred the body to #1817), this rung lets an adopted ref be resolved back to the bytes that produced it. **Store-only:** the adoption route is not yet wired to persist bodies, and no CCL registry/evaluator runtime lands here.

**The declaration route.** The `declare_institutional_domain` handler:

- requires the existing `governance:write` scope;
- derives the declaring actor from the authenticated token subject — never a request-body field;
- accepts the closed-taxonomy `BootstrapEntityType` (out-of-taxonomy → 400) and an optional 64-char hex `CharterId` (length-checked before decoding; malformed → 400);
- calls `GovernanceManager::declare_institutional_domain_gated(...)` — never the ungated bootstrap seam — which resolves authority through `DefaultMandateGate` before any store write, then persists, and returns the declared-domain projection.

It intentionally does **not** apply `check_domain_membership`: declaration may be the act that *establishes* the governed domain, so requiring prior membership could wrongly block the first authorized declaration. The `DefaultMandateGate` declare grant (domain-scoped, `institutional_domain:declare`) is the decisive — and strictly stronger — authority check. Error mapping: gate denial → 403; already declared → 409; missing backend/store or gate backend fault → 500; malformed body / `entity_type` / `charter_id` → 400. Because the gate resolves before the duplicate check, an unauthorized attempt on an already-declared domain returns **403, not 409** — no existence leak.

**The adoption route.** The `adopt_domain_policy` handler:

- requires the existing `governance:write` scope;
- enforces the existing coarse `check_domain_membership` guard;
- derives the adopting actor from the authenticated token subject — never a request-body field;
- content-addresses the candidate policy server-side from `policy_content` (bounded by `MAX_DOMAIN_POLICY_CONTENT_BYTES`) and binds it to the path `domain_id`, so the policy↔domain identity is correct by construction (the pure-core `PolicyForOtherDomain` rejection is unreachable from the route);
- calls `GovernanceManager::adopt_domain_policy_persisted_with_body(..., Some(policy_content), ...)` (#2180), which loads the `InstitutionalDomain`, resolves authority through `DefaultMandateGate` (the authoritative adoption check — coarse membership is additive, not a substitute), applies the pure-core `InstitutionalDomain::adopt_policy()`, **persists the raw `policy_content` body by its content-addressed `DomainPolicyId`** (the #2179 body store), saves the domain, and returns the adopted `DomainPolicyRef` projection. The body write runs **after** the gate resolves and **before** the `current_policy` update, so a rejection persists neither; the adopted ref resolves back to its body via `GovernanceManager::get_domain_policy_body(&DomainPolicyId)`. No HTTP body-read route is added in this rung.

Error mapping: a missing `GovernanceDomain` (the `check_domain_membership` guard runs first) **or** an undeclared `InstitutionalDomain` → 404; missing store / backend read-write failure → 500; a non-member (membership guard) or a gate / pure-core structural authority refusal → 403; empty/whitespace or oversize `policy_content`, or a malformed body/DID → 400.

**Concurrency.** `adopt_domain_policy_persisted_with_body` (and its body-less `adopt_domain_policy_persisted` wrapper) serializes its `load → gate-adopt → (optional save body) → save domain` critical section **per domain** via an in-process lock keyed by `GovernanceDomainId` (`GovernanceManager::domain_adoption_lock`), so two concurrent same-domain adoptions cannot last-writer-win and drop an intervening `current_policy` update; different domains stay concurrent, and the lock is never held across an `.await`. This is a **single-node** guarantee only — a transactional / multi-writer store primitive (atomic compare-and-swap `get`+`put`) for cross-process safety remains future work.

**Status and forward work.** Both gated HTTP surfaces now exist (adopt #2172, declare #2176) over the mandate-gated manager seams, and the flagged ADR-0083 sub-question (declaration is an authority-bearing act) is settled in favor of gating. The #2142 MVP — minimal `InstitutionalDomain` / `DomainPolicy` runtime root with gate-resolved declare + adopt — is in place, and **#2142 was closed-completed on 2026-06-23**. The first follow-on rung (#2178) added the durable `DomainPolicy` **body** store seam described above, and #2180 **wires it into adoption**: the persisted adoption path now saves `policy_content` by `DomainPolicyId` (after the gate, before the `current_policy` update) and exposes a manager-level resolve (`get_domain_policy_body`), so an adopted `current_policy` ref resolves back to its body. Forward work, explicitly **not** landed: a public HTTP body-read route; the CCL policy registry / evaluator selection / CCL evaluation (#1817); the full `InstitutionalDomain` / `DomainPolicy` lifecycle (standing, services, routing, federation, exit); the service-binding runtime; package activation; entity-aware auth cutover; and cross-process / multi-writer transactional safety (the per-domain lock is single-node only).
