...
> **Status (#2142).** Both gated HTTP routes have landed: domain-policy **adoption** (`POST .../domain-policy/adopt`, #2172) and `InstitutionalDomain` **declaration** (`POST .../institutional-domain/declare`, #2176), the latter over the mandate-gated manager seam from #2174. #2142's MVP acceptance criteria appear satisfied; full lifecycle (standing, services, routing, federation, exit), durable `DomainPolicy` body storage, and CCL evaluation remain forward work. #2142 stays open pending explicit acceptance review.
