...
**Still open / explicitly not landed.** Both gated HTTP surfaces now exist (adopt #2172, declare #2176) over the mandate-gated manager seams, and the flagged ADR-0083 sub-question (declaration is an authority-bearing act) is settled in favor of gating. The #2142 MVP — minimal `InstitutionalDomain` / `DomainPolicy` runtime root with gate-resolved declare + adopt — is in place. Forward work, explicitly **not** landed: the full `InstitutionalDomain` / `DomainPolicy` lifecycle (standing, services, routing, federation, exit); durable `DomainPolicy` **body** storage and the CCL policy registry / evaluator selection / CCL evaluation (#1817); the service-binding runtime; package activation; entity-aware auth cutover; and cross-process / multi-writer transactional safety (the per-domain lock is single-node only). **#2142 remains open pending explicit acceptance review.**
