...
| Minimal runtime root for `InstitutionalDomain` / `DomainPolicy` (Declare + Adopt slice) | `ADR-0083` (#2142). Declare + Adopt-policy slice **landed** in `icn-governance` (`institutional_domain` module: `InstitutionalDomain`, `DomainPolicy` / `DomainPolicyRef`, fail-closed `adopt_policy`; tests cover declare, adopt, structural inertness, and missing/ambiguous/unbound/inactive authority). App-side adoption is now **gate-resolved**: `apps/governance::domain_policy_adoption::adopt_domain_policy_gated` runs the existing `DefaultMandateGate::require()` (actor → active grants → `TypedScope.domain` + Execution class + `domain_policy:adopt` act token + mandate lifecycle) before the pure-core structural commit (#2142 follow-up). The adoption seam is now reachable at the governance app boundary via `GovernanceManager::adopt_domain_policy` (#2166), persisted via `GovernanceManager::adopt_domain_policy_persisted` over the `InstitutionalDomain` store (#2170), and **exposed over HTTP** at `POST /gov/domains/{domain_id}/domain-policy/adopt` (#2172). Full lifecycle (standing, services, routing, federation, exit) and CCL evaluation remain forward work; **no declare/create route exists** (declare is not yet mandate-gated) — see "Domain-policy adoption: app boundary and HTTP route" below. |
...
> **Status (#2142).** The persisted, gated domain-policy adoption **HTTP route has landed** (#2172). #2142 remains open: the route is one adoption surface, not the full lifecycle, and **no declare/create route exists**.
...
**The route.** The `adopt_domain_policy` handler:
...
**Still open / explicitly not landed.** **No declare/create route exists:** `declare_institutional_domain(...)` persists a domain but is **not mandate-gated** yet (a flagged ADR-0083 sub-question), so it must not be exposed on a routable surface as-is — exposing it would be an authority bypass. Also forward work: the full `InstitutionalDomain` / `DomainPolicy` lifecycle (standing, services, routing, federation, exit); durable `DomainPolicy` **body** storage and the CCL policy registry / evaluator selection / CCL evaluation (#1817); the service-binding runtime; package activation; and cross-process transactional adoption safety. **#2142 remains open.**
