...
- calls `GovernanceManager::adopt_domain_policy_persisted(...)`, which loads the `InstitutionalDomain`, resolves authority through `DefaultMandateGate` (the authoritative adoption check — coarse membership is additive, not a substitute), applies the pure-core `InstitutionalDomain::adopt_policy()`, saves the domain, and returns the adopted `DomainPolicyRef` projection.
