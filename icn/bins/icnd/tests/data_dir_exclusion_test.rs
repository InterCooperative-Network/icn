...
/// A supplementary group this process belongs to, for constructing an on-disk
/// identity that differs from the one new files receive — without privilege.
#[cfg(unix)]
...
fn chgrp(path: &Path, gid: u32) {
    ...
    assert_eq!(rc, 0, "fixture: chown failed");
}

/// The daemon must ask the same account question `icnctl` asks before creating
/// a lock.
///
/// `acquire` creates `.icn-data-dir.lock`, retains it after release, and gives
/// it mode 0600. Launched as the wrong account against a service-owned root —
/// including a start that then fails for an unrelated reason — it would leave a
/// file the owning account cannot reopen, and this crate fails closed on
/// exactly that, so the service would never start again until repaired.
///
/// `icnctl`'s creating acquisitions have asked this for several commits. The
/// daemon's did not, because the rule lived in `icnctl`.
#[cfg(unix)]
#[test]
fn the_daemon_refuses_to_create_its_lock_under_the_wrong_account() {
    let Some(other_gid) = a_supplementary_group() else {
        eprintln!(
            "SKIPPED the_daemon_refuses_to_create_its_lock_under_the_wrong_account: \
    ...
    // Generated by the daemon itself. The guard sits after the configuration is
    // loaded — it locks `config.data_dir`, which is not known before that — so a
    // deliberately unloadable skeleton would fail earlier and prove nothing
    // about the account check (icn#2747 is why hand-written ones do not load).
    let init = std::process::Command::new(icnd_bin())
        .args(["--init", "--node-name", "account-guard-fixture"])
        .arg("--data-dir")
        .arg(dir.path())
        .env("ICN_KEYSTORE_PASSPHRASE", "test-only-not-a-real-credential")
        .output()
    ...
    assert!(
        !lock.exists(),
        "fixture: the storage lock must be absent so this exercises *creation*"
    );
    ...
    assert!(
        !ok,
        "the daemon must refuse under the wrong account:\n{text}"
    );
    assert!(
        text.contains("cannot use"),
        "and must say why, naming the account mismatch:\n{text}"
    );
    assert!(
        !lock.exists(),
...
/// `--validate-config` joins the reader domain when there is a lock to join.
///
/// Validation reads `icn.toml`, and `Config::to_file` rewrites it with a plain
/// `fs::write`; overlapping a `federation` edit it could read inside the
/// truncate window and reject a valid configuration on timing alone.
///
/// Deterministic: the writer's lock is held exclusively by this test process
/// for the whole invocation, so there is no race to lose. The companion test
/// `validate_config_works_in_a_directory_the_lock_cannot_be_created_in` pins
/// the other half — with no lock to join, validation still proceeds.
#[cfg(unix)]
#[test]
fn validate_config_joins_an_existing_configuration_lock() {
    let dir = tempfile::TempDir::new().unwrap();
    let config = write_unloadable_config(dir.path());

    let writer =
        icn_core::DataDirLock::acquire_config(dir.path(), "a configuration writer").unwrap();
    ...
    assert!(
        !out.status.success(),
        "validation must contend with a configuration writer rather than reading past it:\n{text}"
    );
    assert!(
        text.contains("already holds") || text.contains("configuration"),
