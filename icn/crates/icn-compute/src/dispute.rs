...
impl ComputeResult {
    ...
    pub fn verify_signature(&self) -> Result<()> {
        ...
        let verifying_key = self.executor.to_verifying_key()
            .map_err(|e| ComputeError::InvalidResult(format!("Failed to extract public key: {e}")))?;
        ...
        verifying_key.verify(&self.signing_bytes(), &signature)
            .map_err(|e| ComputeError::InvalidResult(format!("Signature verification failed: {e}")))?;
...
impl DisputeManager {
    ...
    pub fn initiate_dispute(
        ...
        let has_conflict = results.iter().skip(1).any(|r| {
            blake3::hash(&r.output) != first_hash
        });
    ...
    pub fn resolve_by_consensus(&self, dispute_id: &str) -> Result<DisputeResolution> {
        ...
        let (_, majority_group) = result_groups.iter()
...
mod tests {
    ...
    fn test_dispute_initiation() {
        ...
        let dispute_id = manager.initiate_dispute(
            test_task_hash(),
            submitter,
            vec![result1, result2],
        ).unwrap();
    ...
    fn test_dispute_no_conflict() {
        ...
        let err = manager.initiate_dispute(
            test_task_hash(),
            submitter,
            vec![result1, result2],
        ).unwrap_err();
    ...
    fn test_consensus_resolution() {
        ...
        match resolution {
            DisputeResolution::Consensus { majority, total, .. } => {
    ...
    fn test_arbiter_assignment() {
        ...
        // High trust arbiter
        assert!(manager.assign_arbiter(&dispute_id, arbiter.clone(), 0.9).is_ok());
        
        // Low trust arbiter should fail
        let low_trust_arbiter = KeyPair::generate().unwrap().did().clone();
        assert!(manager.assign_arbiter(&dispute_id, low_trust_arbiter, 0.3).is_err());
    }
}
