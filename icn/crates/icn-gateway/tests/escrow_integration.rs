use icn_gateway::api::escrow::{
    Escrow, EscrowCondition, EscrowStatus, EscrowStore, check_conditions,
};
...
async fn test_escrow_lifecycle() {
    ...
    let mut escrow = Escrow {
        ...
        conditions: vec![EscrowCondition::RequiresApproval { did: charlie.did().to_string() }],
        approvals: Vec::new(),
        expires_at: None,
    ...
    assert!(!conditions_met_bob, "Bob should not be able to fulfill Charlie's approval condition");
    ...
    assert!(conditions_met_charlie, "Conditions should be met after Charlie approves");
