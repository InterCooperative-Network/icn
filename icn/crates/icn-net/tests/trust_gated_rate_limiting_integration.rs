...
async fn test_trust_gated_rate_limiting_full_scenario() -> Result<()> {
    ...
    graph.add_edge(TrustEdge::new(alice.clone(), carol.clone(), TrustScore::unchecked(0.7)))?;
    // Dave: Federated (direct 1.0 -> final 0.7)
    graph.add_edge(TrustEdge::new(alice.clone(), dave.clone(), TrustScore::unchecked(1.0)))?;
...

    {
        ...
        graph.add_edge(TrustEdge::new(alice.clone(), bob.clone(), TrustScore::unchecked(1.0)))?;
    }
...
async fn test_cache_performance() -> Result<()> {
    ...
    let store = Arc::new(SledStore::temporary()?);
    let mut graph = TrustGraph::new(store, alice.clone());
    graph.add_edge(TrustEdge::new(alice.clone(), bob.clone(), TrustScore::unchecked(0.8)))?;
