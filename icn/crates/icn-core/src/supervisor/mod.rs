            ...
            *network_handle_for_handler.write().await = Some(network_handle.clone());
                    ...
                    move |topic, entry, _subscriber_did| {
                        ...
                        // Handle distributed snapshot coordination
                            ...
                            match bincode::serde::decode_from_slice::<
                                icn_snapshot::SnapshotMessage,
                                _,
                            >(
                                &entry_data, bincode::config::legacy()
                            )
                            .map(|(v, _)| v)
                            {
                                Ok(snapshot_msg) => {
            ...
            governance_event_subscription = Some({
                ...
                event_bus.subscribe(Arc::new(move |event| {
                    match event {
                        SystemEvent::ProposalAccepted { proposal_id, payload, decided_at, domain_id, .. } => {
                            let domain_id = domain_id.clone(); // Clone for use in async blocks
                            match &payload {
                                ...
                                ProposalPayload::ConfigChange { new_config } => {
                                    ...
                                    match serde_json::from_str::<icn_governance::GovernanceConfig>(new_config) {
                                        Ok(parsed_config) => {
                                            ...
                                            let prop_id = proposal_id.clone();
                                            let dom_id = icn_governance::GovernanceDomainId::new(domain_id.clone());
