...
impl Pkcs11Backend {
    ...
    ///
    /// **UNIMPLEMENTED**: This backend cannot return IdentityBundle until
    /// IdentityBundle is refactored to use DidKey instead of KeyPair.
    ///
    /// Hardware-backed keys never expose secret material, so they cannot
    /// be represented as KeyPair. This method will return an error until
    /// the necessary refactoring is complete.
    ///
    /// # Arguments
    /// * `pin` - HSM user PIN
    ///
    ...
    ///
    /// For PKCS#11-backed keys, the secret key material never leaves the HSM.
    /// Therefore, constructing a software `KeyPair` is not supported and this
    /// function will return an error instead of creating a dummy keypair.
    ///
    /// # Error
    ///
    /// This function always returns an error. Hardware-backed keys should use
    /// the `DidKey::Hardware` variant instead of `KeyPair`.
    fn create_hsm_keypair(&self, _verifying_key: &VerifyingKey) -> Result<KeyPair> {
        anyhow::bail!(
            "Cannot create KeyPair for PKCS#11 backend: secret key material \
             does not leave the HSM. Use DidKey::Hardware representation instead. \
             IdentityBundle refactor required before this backend can be used."
        )
    }
