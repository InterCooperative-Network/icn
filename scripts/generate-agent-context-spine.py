...
PATH_GUIDANCE = [
    ...
    ("gateway", "icn/crates/icn-gateway/", "icn-gateway (gateway / route / API surface)",
     ...
     ["python3 docs/scripts/route_inventory.py --check",
      ...
      "cd icn && cargo test -p icn-gateway --features sled-storage",
      "If routes/API shape change, regenerate OpenAPI: cd icn && cargo build -p icnctl && ./target/debug/icnctl api export-openapi -o ../docs/api/openapi.generated.yaml",
      "If the API shape changed, regenerate + check SDK types: cd sdk/typescript && npm ci && npm run generate-types && npm run check-types"],
    ...
    ("truth-spine", "ops/state/truth/", "Canonical truth spine (fact ownership)",
     ...
     ["skill:navigator"], ["agent:icn-architect", "agent:icn-code-reviewer"],
     "ops/state/truth"),
    ("sdk-ts", "sdk/typescript/", "TypeScript SDK (build/test/lint + generated-types drift)",
     ["Run the full package gates for code edits: npm ci, build, test, lint",
      "Generated types: commit only regenerated files under src/generated/*",
     ...
     ["cd sdk/typescript && npm ci && npm run build && npm test && npm run lint",
      "Generated types only (after a gateway API change), commit only src/generated/*: cd sdk/typescript && npm ci && npm run generate-types && npm run check-types"],
