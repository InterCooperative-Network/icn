...
if [ "$rc" -ne 0 ] && grep -qE "only [0-9]+ of [0-9]+ checks ran" "$TMP/nodist.log"; then
...
if [ -n "$BASE_COUNT" ] && grep -q "EXPECTED_CHECKS = $BASE_COUNT" "$GATE"; then
  ok "EXPECTED_CHECKS ($BASE_COUNT) matches what a complete run actually performs"
else
  bad "EXPECTED_CHECKS does not match the observed count" "observed=$BASE_COUNT"
fi
...
if grep -q "cursor" "$TMP/nocursor.log" && grep -q "missing: .cursor/mcp.json" "$TMP/nocursor.log"; then
  ok "coverage prose is accompanied by the failure when the claim is untrue"
else
